...
j=1;
cc=1;
for i=1:length(Membrane_C_strct) %creates Ra variation, Rm tables of each Ra sample per each cell
...
[M2,N2]=size(master_metadata);

for i=1:M
exclusion_report{i+M2,1}=Parameter_strct(i).parameter;
end

exclusion_report(1:M2,1:N2)=master_metadata;
